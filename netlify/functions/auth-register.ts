...
import { createApiResponse, createApiError, isValidEmail } from '../../src/lib/utils';
...
const handleRegister = async (event: any, context: any) => {
  // 只允许POST请求
  ...
  try {
    ...
    if (!canSend) {
      logger.warn('Rate limit exceeded for verification code', { email });
...
export const handler: Handler = async (event, context) => {
  // 简化版本，无中间件包装
  return handleRegister(event, context);
};
